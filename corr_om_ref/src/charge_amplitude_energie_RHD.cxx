...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    */
    ...
    ULong64_t tdc = 0;
    int om_number = 0;
    double charge_tree = 0;
    double amplitude_tree = 0;
    double time = 0;
    TTree *Result_tree;
    Result_tree= new TTree("Result_tree","");
    Result_tree->Branch("om_number", &om_number);
    Result_tree->Branch("charge_tree", &charge_tree);
    Result_tree->Branch("amplitude_tree", &amplitude_tree);
    ...
    */
    ...
    snfee::io::multifile_data_reader rhd_source(reader_cfg);
    snfee::data::calo_hit_record calo_hit; 
    ...
    std::size_t rhd_counter = 0;
    //Main loop
    while (rhd_source.has_record_tag() ) {
      rhd_counter++;
      rhd_source.load(calo_hit);

      //if(rhd_counter < 10)std::clog<<"In Run : "<<run_id<<" Trigger # "<<trigger_id <<std::endl;
	...
	if(rhd_counter == 1) first_tdc = tdc;
	else last_tdc = tdc;
	...
	for (int ichannel = 0; ichannel < snfee::model::feb_constants::SAMLONG_NUMBER_OF_CHANNELS; ichannel++) {
	  ...
	  if (om_num != -1){
	    om_number = om_num;
	    charge_tree = -ch_charge;
	    time = tdc*6.25e-9;
	    amplitude_tree = -((ch_peak)*2500.0/(4096.0*8.0));
	    Result_tree->Fill();
	  }
	  ...
	  //     std::string counter_str = std::to_string(rhd_counter);
	  //     std::string const tmp_title { "event # " + counter_str + " ch : " + readout_id.to_label().to_string()};

	  //     if(rhd_counter == 1 || readout_id == control_readout_id)
    ...
    std::clog<<"Nb of entry  : "<<rhd_counter<<std::endl;
